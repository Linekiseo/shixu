...
以下截图来自当前版本的原生界面，使用独立的演示资料，不包含私人记录，也不会向正式资料库添加示例。

### 全部记录 · 看见不同内容的特点

文字、图片、网页和文档使用各自的卡片形式，按天组织，并支持折叠回看。

![全部记录：按天排列的文字、图片、网页、Markdown 和文档卡片](docs/images/all-records.jpg)

### 内容组合 · 把一件事的线索放在一起

相关内容共用一个组合容器，可以继续添加卡片，并拖动调整组内顺序。

![内容组合：产品设计与阅读想法分组展示，组内保留各类卡片预览](docs/images/compositions.jpg)

### 时间浏览 · 从日历回到当时的记录

在年、月、周、日之间切换；选择日期后，展开当天的时间片段和内容卡片。

![时间浏览：月历展示记录分布，选中日期后呈现当天的时间片段与卡片](docs/images/calendar.jpg)
...
- 仓库不包含个人记录、真实账号、API Key、浏览器会话、网页存档或私人工作截图；README 中的界面截图仅使用隔离演示资料。
